...
Run each file in sequence (see **`migrations/README.md`** for order, idempotency, optional `verify_schema.sql`, **`migrations/SEEDS.md`** for bcrypt / seed data, **`migrations/LEGACY_AND_RESIDENTS.md`** for legacy `patient` / new `residents` alignment, and **`migrations/DEV_REFERENCE.md`** for column names / FKs for other developers). In phpMyAdmin: select `mimds` → **Import** tab → choose file.
